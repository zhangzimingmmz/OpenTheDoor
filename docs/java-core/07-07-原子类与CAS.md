...
## 目录
- [1. 为什么需要原子类](#1-为什么需要原子类)
- [2. CAS原理](#2-cas原理)
- [3. AtomicInteger详解](#3-atomicinteger详解)
- [4. 其他原子类](#4-其他原子类)
- [5. ABA问题](#5-aba问题)
...
**关键点：**
- 使用`volatile`保证可见性
- 使用`Unsafe.compareAndSwapInt()`实现CAS
- 自旋直到CAS成功
...
在高并发场景下，`LongAdder`性能优于`AtomicLong`：
...
**上一章：** [07-06 - 并发集合](./07-06-并发集合.md)  
**下一章：** [07-08 - 并发设计模式 →](./07-08-并发设计模式.md)
