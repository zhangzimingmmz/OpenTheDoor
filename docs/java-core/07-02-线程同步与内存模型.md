...
## 目录
- [1. 为什么需要同步](#1-为什么需要同步)
- [2. synchronized关键字](#2-synchronized关键字)
- [3. volatile关键字](#3-volatile关键字)
- [4. Java内存模型（JMM）](#4-java内存模型jmm)
- [5. happens-before规则](#5-happens-before规则)
- [6. synchronized和volatile的区别](#6-synchronized和volatile的区别)
- [7. 最佳实践](#7-最佳实践)
- [8. 面试高频问题](#8-面试高频问题)
...
`synchronized`关键字提供了一种**互斥锁（Mutex Lock）**机制，保证同一时刻只有一个线程能执行被保护的代码块。

**作用：**
1. **原子性（Atomicity）** - 保证操作的原子性
2. **可见性（Visibility）** - 保证变量的可见性
3. **有序性（Ordering）** - 保证一定的执行顺序
...
`volatile`关键字保证变量的**可见性**和**有序性**，但不保证**原子性**。
...
**内存区域：**
1. **主内存（Main Memory）** - 所有线程共享，存储共享变量
2. **工作内存（Working Memory）** - 线程私有，存储变量的副本
...
**答案：** 见[6. synchronized和volatile的区别](#6-synchronized和volatile的区别)

### Q2: volatile能保证原子性吗？

**答案：** 不能。volatile只保证可见性和有序性，不保证原子性。例如`count++`不是原子操作。
...
**返回：** [07-Java并发编程](./07-Java并发编程.md)  
**上一章：** [07-01 - 线程基础](./07-01-线程基础.md)  
**下一章：** [07-05 - 线程池 →](./07-05-线程池.md)
